...
ZXDev - это мощная подсистема для кроссразработки на Обероне-2 для компьютера Sinlair ZX Spectrum.

Назначение: быстрое средство разработки игр и программ для тех, кто не знает
или не хочет использовать ассемблер Z80, но хочет писать продвинутые игры для ZX Spectrum.
(c) Andrew771
...
http://forum.oberoncore.ru/viewtopic.php?f=29&t=3892
...
BEGIN
  IF Sets.Debug THEN (* Это прямой аналог ifdef'а, однако если Debug = FALSE,
    ...
    IF ~ ((x IN {0..31}) & (y IN {0..22})) THEN Debug.Stop("Bad sprite coords!") END;
  END;
  ...
END PutSprite;

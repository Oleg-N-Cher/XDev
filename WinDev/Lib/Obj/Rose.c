...
static BOOLEAN Rose_Resolve (Rose_Socket *self, LONGINT *self__typ, CHAR *adr, LONGINT adr__len)
{
	if (LibC_sscanfi((SYSTEM_PTR)((INTEGER)adr), (SYSTEM_PTR)((INTEGER)":%d"), &(*self).port) == 1) {
		(*self).host[0] = 0x00;
		(*self).ip[0] = 0x00;
		(*self).ipv4 = 0;
		return 1;
	} else if (LibC_sscanfai((SYSTEM_PTR)((INTEGER)adr), (SYSTEM_PTR)((INTEGER)"%[^:]:%d"), (SYSTEM_PTR)((INTEGER)(*self).host), &(*self).port) == 2) {
		if (Rose_ResolveHost(&*self, self__typ)) {
			return 1;
		}
	} else if (LibC_sscanfi((SYSTEM_PTR)((INTEGER)adr), (SYSTEM_PTR)((INTEGER)"%d"), &(*self).port) == 1) {
		(*self).host[0] = 0x00;
		(*self).ip[0] = 0x00;
		(*self).ipv4 = 0;
		return 1;
...
void Rose_Listener_Listen (Rose_Listener *self, LONGINT *self__typ, CHAR *localAdr, LONGINT localAdr__len)
{
	...
	if ((*self).handle != (SYSTEM_PTR)-1) {
		Ignore_Ptr(LibC_memset((SYSTEM_PTR)((INTEGER)&sock_addr), 0, (SYSTEM_PTR)16));

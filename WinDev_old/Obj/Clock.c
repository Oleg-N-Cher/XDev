	...
	switch (message) {
		case 274: 
			if (wParam == (SYSTEM_PTR)1001) {
				Ignore_Int(WinApi_DialogBoxParam(Clock_Instance, (SYSTEM_PTR)((INTEGER)"#1"), wnd, Clock_AboutProc, NIL));
			} else {
...
static BOOLEAN Clock_ClockInit (void)
{
	...
	clockClass.lpszClassName = (SYSTEM_PTR)((INTEGER)Clock_szBuffer);
...
/* BEGIN */
	...
	Ignore_Int(WinApi_LoadString(WinApi_Instance(), 2, (SYSTEM_PTR)((INTEGER)Clock_szBuffer), 30));
	...
	Clock_Wnd = WinApi_CreateWindowEx(0x0, (SYSTEM_PTR)((INTEGER)Clock_szBuffer), (SYSTEM_PTR)((INTEGER)Clock_szBuffer), 0xcf0000, (-2147483647-1), 0, Clock_width, Clock_height, NIL, NIL, WinApi_Instance(), NIL);
	...
	if (WinApi_SetTimer(Clock_Wnd, Clock_TimerID, 450, (WinApi_TIMERPROC)0) == 0) {
		...
		Ignore_Int(WinApi_MessageBox(NIL, (SYSTEM_PTR)((INTEGER)&Clock_pszTooMany), (SYSTEM_PTR)((INTEGER)Clock_szBuffer), 0x1010));
		Clock_DeleteTools();
		__HALT(1);
	}
	Ignore_Int(WinApi_LoadString(WinApi_Instance(), 1, (SYSTEM_PTR)((INTEGER)Clock_szBuffer), 30));
	Clock_Menu = WinApi_GetSystemMenu(Clock_Wnd, 0);
	Ignore_Int(WinApi_ChangeMenu(Clock_Menu, 0, (SYSTEM_PTR)((INTEGER)Clock_szBuffer), 1001, 0x0100));
